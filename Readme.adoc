...
To join the VPN just add your device to `machines.nix` (it becomes a `meta.machines.<hostname>` entry). There is a script for that in this repo. cd into the root of this repo and run
...
Secrets are managed with `geheimnix`. They are stored in the secrets directory.
...
- Upstream the part that chmods the paths created by geheimnix
...
1. Point `meta.services.mail.host` at the new host in `machines.nix`
...
6. Adjust the config.
7. The setup-host script should have given you a command to run like `nixos-anywhere --extra-files /tmp/tmp.absdfsad --flake '.#glouble' root@192.168.178.15 --no-substitute-on-destination --build-on local`
8. Encryption: Place the encryption secret at /tmp/secret/tmp/secret.key on the target machine. You can probably use nixos-anywhere for this
9. Secureboot: Generate keys with `sbctl --disable-landlock --debug create-keys --database-path /tmp/secret/var/lib/sbctl/GUID --export /tmp/secret/var/lib/sbctl/keys` ??? and more
...
=== Provision an installer

1. Generate the iso with `nix run .#generate-installer`
2. Create a bootable USB drive with `dd if=installer.iso of=/dev/sdX bs=4M status=progress && sync`

for d in /dev/disk/by-id/ata-*[^part1] /dev/nvme?n1 ; do echo Formatting $d ; (echo -e "g\nw\n" | fdisk $d &) ; done  ; sleep 1 ; while wait -n; do : ; done ; echo done